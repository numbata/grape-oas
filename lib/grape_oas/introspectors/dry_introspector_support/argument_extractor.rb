    ...
    module DryIntrospectorSupport
      ...
      module ArgumentExtractor
        ...
        def range_to_enum_array(range)
          ...
          # Reject ranges that can't report size (e.g., non-discrete ranges)
          return nil unless range.respond_to?(:size) && range.size.is_a?(Integer)

          # Reject excessively large ranges
          return nil if range.size > MAX_ENUM_RANGE_SIZE
